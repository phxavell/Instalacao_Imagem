...
def main():
    ...
    powerflip = False
    logstep('00300 - Fonte de alimentacao externa!!!')
    ...
    if len(notebook.get('serialnumber')) != 11:
        color('red')
        print(
            '''Numero de serie com tamanho errado (Chamar Gabriel ou Trober)'''
        )
        sys.exit(5)
    ...
    if not notebook.get('serialnumber')[6:11].isnumeric():
        color('red')
        print(
            '''Numero de serial nao parece ser numericamente valido (Chamar Gabriel ou Trober)'''
        )
        sys.exit(5)
    ...
    if int(notebook.get('serialnumber')[6:8]) > 54:
        color('red')
        print(
            '''Numero da semana estah errado (Chamar Gabriel ou Trober)'''
        )
        sys.exit(5)
...
"%s"''' % notebook.get('bios_name')
    ...
    # ??? Desligado dism abaixo para windowws 11
    ...
    command = 'z:\\scripts\\aplica_imagem_etapa_virtuo.bat'
