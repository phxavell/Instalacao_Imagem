...
import av_art
import av_factory_microsoft_test
import av_os_image
            ...
            if (var_os[:4]== 'INTR'): # Feito para identificar quando a máquina for uma interna
                ...
                if hwz:
                    ...
                    if (notebook.get('code')[:3] == 'A55'):
                        ...
                        command = '''Z:\\scripts\\clevo\\H2OSDE-Wx64.exe -BS %s''' % notebook.get('ordemservico')   
                    elif (notebook.get('code')[:4] == 'BOLB'):     ################################################### --->>> ADICIONANDO O B.ON LITE NEW, CODIFICAÇÃO FICARÁ EM :4 POIS JÁ EXISTE UMA CODIFICAÇÃO EM :3
                        command = '''Z:\\scripts\\clevo\\H2OSDE-Wx64.exe -BS %s''' % notebook.get('ordemservico')              
                    else:
                        command = '''Z:\\Scripts\\TFGTools\\AMIDEWINx64.EXE /BS %s''' % notebook.get('ordemservico')
                        print("DEBUG Command: %s" %command)
                        
                    operation = av_factory_microsoft_test.cmdexec(command)
...
###         VERIFICAR ONDE FICARA O ARMAZENADO OS LOGS COMPARE PARA OS NOVOS PRODUTOS 2023
...
############################################################################################################################################################    
 ...
 ##################################################################################################################################################################   
    ...
    t1 = Thread(target=av_factory_microsoft_test.cmdexec,args=(command,))
    t1.start()
    ...
    if command:
        operation = av_factory_microsoft_testy.cmdexec(command)
