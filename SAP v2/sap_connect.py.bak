...
def description(var_os):
    ...
    bbb_dic = bbb.json()
    return bbb_dic['DocumentLines'][0]['ItemDescription']
...
def virtuo(var_os):
    ...
    bbb = session.get(url2)
    bbb_dic = bbb.json()
    return bbb_dic['DocumentLines'][0]['U_AVELL_SIS_Virtuo'] # Retorna se o sistema é Virtuo ou não
...
def sistema(var_os):
    ...
    bbb = session.get(url2)
    bbb_dic = bbb.json()
    var_sis = bbb_dic['DocumentLines'][0]['U_AVELL_SO']
