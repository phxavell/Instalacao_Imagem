...
def cod_item(var_ped):
    url2 = "https://avell.ramo.com.br:50000/b1s/v1/ProductionOrders?$filter=DocumentNumber eq %s" % var_ped #Passa o numero da OS como parametro para retornar o numero da NB
...
def sistema(var_os):   
    pdb.set_trace()                         #necessário atenção para verificar se com windows ou não para pedidos com mais de 1 máquina.
    if (var_os[:4] == 'INTR'):
        return 'Windows 10 PRO'
    pd = pedido(var_os)
    url2 = "https://avell.ramo.com.br:50000/b1s/v1/Orders(%s)?$select=DocumentLines" % pd # Passa por parametros a ordem de venda para buscar a descrição do computador. Usar a mesma para tirar o sistema e o modelo do notebook
