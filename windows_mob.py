...
def main():
    ...
    if not hwz:
        ...
        while True:
            ...
            new_serial1 = input("Serial: AVNB")
            new_serial2 = input("\n\nSerial: AVNB")
            if str(new_serial1) == str(new_serial2):
                print("\n\nO novo numero de serie serah:", "AVNB" + str(new_serial2))
                hwz = odoo_hwinfo.process("AVNB" + str(new_serial2))
                if hwz:
                    notebook['serialnumber'] = "AVNB" + str(new_serial2)
...
"%s"''' % notebook.get('bios_name')
    ...
    #command = '''Z:\Scripts\TFGTools\AMIDEWINx64.EXE /BP \"%s"''' % notebook.get('bios_name')
    #operation = av_factory.cmdexec(command)
