...
import a_sap_connect24new as sap
...
def main():
    ...
    if not notebook.get('ordemservico') is None:  #===================================================#
        while True:
            ...
            if hwz:
                ...
                print("Algo deu errado. Chamar José nesse caso!!!")
    ...
    # Verifica se o código do notebook é B147B127X
    if notebook.get('code') == 'B145B125X':
        pass  # Deixa passar, sem aplicar o time.sleep
        
    elif notebook.get('code') == 'B147B127X':
        pass  # Deixa passar, sem aplicar o time.sleep    
    else:
        if (bios_necessario, ec_necessario) != (notebook.get('bios'), notebook.get('ec')):
            time.sleep(5)
            ...
            print('\033[94m' + '\n   VERSAO DE BIOS E EC NECESSARIOS\n' + '\033[0m')
            print(f"\033[94mBIOS: {bios_necessario} | EC: {ec_necessario}\033[0m")
            time.sleep(9999)  # Aplica o time.sleep se o código não for B147B127X
...
############################################################################################################################################################    
 ...
 ##################################################################################################################################################################   
    ...
    if 'Dexis' in notebook.get('os') or 'Dexis' in notebook.get('os'):
        if 'ION A52i' in sap.description(notebook.get('ordemservico')):
            command = "Dism /apply-image /imagefile:Z:\\windows_images\\DEXIS\\WIN11PRODEXIS_Final.wim /index:1 /ApplyDir:W:\\"
            os.system(command)
            print("[DEXIS] Imagem DEXIS ION A52i")
            regra_virtuo_aplicada = True        
