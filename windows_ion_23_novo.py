...
def main():
         ...
         while True:
            ...
            if (var_os[:4]== 'INTR'): # Feito para identificar quando a máquina for uma interna
                ...
                if hwz:
                    ...
                    #elif (notebook.get('code')[:3] == 'BOL'):    -  BON_ANTIGO
                    #    command = '''Z:\\scripts\\clevo\\H2OSDE-Wx64.exe -BS %s''' % notebook.get('ordemservico')
                    #    print("DEBUG Command: %s" %command)
                    else:
                    ...
                    print("Algo deu errado. Chamar Suporte TI ou Eng.!!!")
    ...
    # #################################################
    # TRACK START #####################################
    # Escrita do numero de serie na placa #############
    logstep("00800 - Writing computer serial number")
    ...
    # ADICIONADO DENTRO DE OEM A ESTRUTURA PARECIDA COM A DO HYB, MAS PRECISO ADICIONAR A ESTRUTURA ESPECÍFICA DO ION
    command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\ION\\OEM\\%s W:\\Users\Administrador\Desktop /e /NFL /NDL /NJH /NJS /nc /ns /np" % notebook.get('code')[:8]
    ...
    ##########ADICIONADO============30/07/2023########################
    elif (notebook.get('code')[:3] == 'A52'):
        command = ""
    ##########ADICIONADO============30/07/2023########################

    # VERSAO ANTERIOR DO B.ON-LITE - NAO VAI ENTRAR NESSA SERIE DO ION
    #elif (notebook.get('code')[:3] == 'BOL'):
    #    command = ""
    elif (notebook.get('code')[:3] == 'BOLB'):
        command = ""
    ...
    # AQUI VOU INSERIR OS SCRIPTS PARA OS NOVOS MODELOS=======================================================================================================

    # AQUI VOU INSERIR OS SCRIPTS PARA OS NOVOS MODELOS=======================================================================================================

    # BON_NEW=================================================================================================================================================
    logstep("01400 - Write BIOS/UEFI selected parameters")
    command = "z:\\construtor\\ION\\ParamBios\\%s.bat " % notebook.get('code')[:8] # Necessario trocar endereco e talvez gerar um codigo novo? Pensar nisso
    # BON_NEW=================================================================================================================================================
    ...
    # Cria logs no Servidor referente as imagens aplicadas.
    if not (notebook.get('ordemservico')[:4] == 'INTR'): # Pra pegar o codigo quando for uma maquina interna
        ecbios_updated = notebook_hwecbios.nb_compare(notebook.get('code'), notebook.get('ec'), notebook.get('bios'))
        if not ecbios_updated:
            time.sleep(5)
    ...
    directory = 'z:\logcompare2023\ion_2023' #ADICIONAR PASTA PARA CADA MODELO NOS SCRIPTS MOB, HYB
    ...
    #with open(filename, 'r') as f:
     ...
     #     print("")
      #    print(" VERSAO DE BIOS CORRETA")
       #   print("")
        #  print(" LIBERADO")
    #else:
     #     print("")
      #    print("VERSAO DE BIOS INCORRETA - VERIFICAR") 
       #   print("")
        #  color('red')
         # time.sleep(99999)
    ...
    if (notebook.get('code')[:3]=='A52'):#Ajustar
        command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\ION\\DriversA52ION k:\\ /e /NFL /NDL /NJH /NJS /nc /ns /np"
    elif (notebook.get('code')[:3]=='A52'):
        command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\ION\\DriversA52NEW k:\\ /e /NFL /NDL /NJH /NJS /nc /ns /np"
    #elif (notebook.get('code')[7] == 'X'):#Utilizar para o Storm X
    ...
    # BON_NEW=====================================================================================================================
        ...
        command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\ION\\DriversBON k:\\ /e /NFL /NDL /NJH /NJS /nc /ns /np"
    elif (notebook.get('code')[7] == 'T'):
        command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\ION\\DriversBON k:\\ /e /NFL /NDL /NJH /NJS /nc /ns /np"
    # BON_NEW======================================================================================================================
    
    #Chamada de Thread para gravacao em paralelo
    ...
    # Verificar para os outros modelos:
    ...
    if (notebook.get('code')[:3]=='A52'):# Unificar 
    ...
    # BON_NEW===============================================================================================================================================
    elif (notebook.get('code')[:8]=='BOLB125T'):
        command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\ION\\www4\\ W:\\Users\Administrador\Desktop\Teste /e /NFL /NDL /NJH /NJS /nc /ns /np"
    # BON_NEW===============================================================================================================================================
    ...
    #origem = r"z:\\construtor\\DesenvolvimentoMao\\ATALHO" - testar com as duas barras o caminho

    # Caminho completo de destino para a copia do diretorio
    destino = r"W:\Users\Administrador\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup"
    #destino = r"W:\\Users\\Administrador\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup" - testar com as duas barras o caminho
