...
def main():
    ...
    else: notebook['serialnumber'] = sap.serial(var_os)
    ...
    if (var_os[:4] == 'INTR'): # Pra pegar o codigo quando for uma maquina interna
        notebook['code'] = var_os[4:12]
    else: 
    ...
    #Verificação de EC
    if not (var_os[:4] == 'INTR'): # Pra pegar o codigo quando for uma maquina interna
    ...
    if notebook.get('code')[:4] == 'INTR':
        command = "x:\\Windows\\System32\\robocopy.exe z:\\UWDrivers\\uwd_%s k:\\ /e /NFL /NDL /NJH /NJS /nc /ns /np" % notebook.get('code')[4:12]
    ...
    command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\MOB\\Teste\\ W:\\Users\Administrador\Desktop\Teste /e /NFL /NDL /NJH /NJS /nc /ns /np"
