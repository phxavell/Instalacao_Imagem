...
def main():
    ...
    if notebook.get('ordemservico') == 'Standard' or notebook.get('ordemservico') == 'M1BC052001C7':
        ...
        while True:
            ...
            if (var_os[:4]== 'INTR'): # Feito para identificar quando a máquina for uma interna
                ...
                if hwz:
                    print("DEBUG CODE: %s" %notebook.get('code'))
                    notebook['ordemservico'] = var_os
    ...
    #notebook['placa-mae'] = notebook_hwcode.nb_placa(notebook.get('code'))
    
    
    #print(notebook.get('placa_mae'))
    print(
    ...
    ##LOGS PARA COMPARADOR DE TESTE - PARA AMAURY - FIZ 27/01/2023 - INÁCIO
    ...
    directory = 'z:\logcompare2023\mob_2023' #ADICIONAR PASTA PARA CADA MODELO NOS SCRIPTS MOB, HYB
    ...
    #arquivo = open('z:\\logcompare2023\imagem\','a')
    #arquivo.write(f'''\n\n OS: {notebook.get('ordemservico')} \n DESCRIÇÃO: {sap.description(notebook.get('ordemservico'))} \n Sistema Operacional: {notebook.get('os')} \n NS: {notebook.get('serialnumber')} \n Data e hora: {datetime.datetime.now()}''' + "\n")
    #arquivo.close()
    ...
    #arquivo = open('z:\\logcompare2023\sistema.txt','a')
    #arquivo.write(f'''\n\n OS: {notebook.get('ordemservico')} - {notebook.get('serialnumber')} - {notebook.get('os')} - {datetime.datetime.now()}''' + "\n")
    #arquivo.close()
    ...
    if "Virtuo" in notebook.get('os'):
       command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\MOB\\VirtuoDesktop\\ W:\\Users\DWOS\Desktop /e /NFL /NDL /NJH /NJS /nc /ns /np"
    if command:
        operation = av_factory.cmdexec(command)
    ...
    if "Virtuo" in notebook.get('os'):
       command = "x:\\Windows\\System32\\robocopy.exe z:\\construtor\\MOB\\Virtuo\\ W:\\Users\DWOS\Desktop\Teste /e /NFL /NDL /NJH /NJS /nc /ns /np"
